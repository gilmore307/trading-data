...
Without `--execute-live-fetch` it returns a plan-only receipt. With `--execute-live-fetch` it performs bounded calendar maintenance. The checked-in systemd unit passes `--skip-trading-economics`, so the timer refreshes Nasdaq earnings schedule and NYSE official exchange-calendar artifacts without appending hourly current/future-month TE source runs. Run the wrapper manually without `--skip-trading-economics` only for a reviewed TE source-refresh window. The NYSE artifact is used to overlay official NYSE/Nasdaq holiday and early-close rows into `trading_data.calendar_market_session`; the calendar-maintenance routes do not admit rows into the M06 event pool. The timer reads `/root/projects/trading-storage/main/shared/equity_total_symbol_pool.symbols.txt` by default, and `TRADING_DATA_CALENDAR_SYMBOLS_FILE` in `/etc/default/trading-data-calendar-maintenance` may override that path.
