...
`trading_data.option_chain_state_source` accepts manager-supplied `params.underlying`, `params.snapshot_time`, optional `params.window_start` / `params.window_end`, and option-chain scope controls, calls the ThetaData option selection snapshot interface once, and writes one contract-level source/cache row per visible contract per returned minute. Historical training acquisition is prepared by the manager before Layer 3 as regular-session 30-minute window requests; provider policy caps remain `1d` while each task key carries the narrower 30-minute `window_start` / `window_end`. The resulting shared SQL rows serve Layer 3 target-level reduction and Layer 9 option-expression candidate preparation through `snapshot_time` range reuse.
