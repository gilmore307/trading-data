...
- **Request parameters:** optional `query_terms`, `topic_categories`, `start_date`, `end_date`, `lookback_days`, `max_rows`, `search_fields`, `focus`, `source_domain_allowlist`, `source_domain_contains`, `impact_scope_hint`. If `query_terms` is omitted, `topic_categories` defaults to `politics,economy,war,technology`.
