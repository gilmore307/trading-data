...
def flush_month_store(base_dir: Path, store: dict[str, dict[int, dict[str, Any]]], *, dataset_name: str) -> None:
    ...
    for month, rows in store.items():
        ...
        if ordered:
            write_month_dir_meta(out_dir, "quotes", ordered[0])
...
def normalize_quote(*, asset_class: str, feed_scope: str, symbol: str, row: dict[str, Any]) -> dict[str, Any]:
...
def fetch_historical_quotes(*, asset_class: str, symbol: str, start: str, end: str, limit: int, resume: bool, output_dir: Path | None) -> dict[str, Any]:
    ...
    rows = obj.get("quotes", {}).get(symbol, [])
    ...
    kept = 0
    for raw in rows:
        row = normalize_quote(asset_class=asset_class, feed_scope=feed_scope, symbol=symbol, row=raw)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch Alpaca historical quotes into symbol/month JSONL partitions.")
...
def main() -> None:
    ...
    result = fetch_historical_quotes(
