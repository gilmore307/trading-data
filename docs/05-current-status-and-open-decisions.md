...
- 2026-04-09: `official_us_core` calendar work now includes an explicit search-backed fallback artifact path so future release events can carry row-level provenance fields such as `calendar_source_type`, `calendar_source_name`, `source_url`, `source_title`, and `last_verified_at` even when fully source-backed parsing is not yet available for every source family
- 2026-04-09: `official_us_core` now also has a first multi-source merge builder that combines maintained and search-backed calendar inputs into one canonical merged artifact using source-priority-based de-duplication
