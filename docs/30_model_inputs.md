...
`trading_data.m09_option_expression_data_acquisition` accepts manager-supplied `params.underlying`, `params.snapshot_time`, and optional `params.snapshot_type` (`entry`/`exit`, default `entry`), `params.max_dte` (`45` default), `params.strike_range` (`5` default), and `params.option_bucket_policy_ref` (`LAYER_09_OPTION_BUCKET_STRIKE_POLICY` default), calls the ThetaData option selection snapshot interface, and writes one row per visible option contract per snapshot. `snapshot_time` is the point-in-time clock; quote/IV/Greeks provider row timestamps are intentionally omitted from the business table. `trading_data.m09_option_expression_feature_generation` turns accepted snapshot rows into deterministic source-only option-candidate payloads for moneyness, spread/liquidity, IV, Greeks, and quality diagnostics.
