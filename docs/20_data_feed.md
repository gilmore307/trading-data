...
`fetch_tradingview_equity_screener.py` captures a bounded no-login TradingView US common-stock screener snapshot for volume, market cap, and sector evidence. `fetch_etf_universe_holdings.py` reads the reviewed Layer 2 ETF universe, calls the official issuer holdings feed for each ETF, filters to equity-like holdings, and writes one deduped holdings CSV for pool construction. `build_equity_total_symbol_pool.py` builds the shared total-pool ledger from TradingView screener CSVs, Layer 2 ETF holdings CSVs, and an optionable-underlying symbol list. The CSV keeps every observed symbol row, including filtered rows marked `inactive`; only `active` rows are written to the adjacent calendar symbols text file. `refresh_equity_total_symbol_pool_from_tradingview.py` runs the TradingView snapshot plus Layer 2 ETF holdings collection by default and rebuilds the shared pool. Until a strict optionable source is supplied, its default mode keeps rows marked `uncertain_verify_before_use` and includes active uncertain rows in the calendar symbol text file; this is calendar/event-monitoring scope only, not proof that listed options are available for trading.
