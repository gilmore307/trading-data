...
| Trading Economics calendar web | Recent/future macro calendar rows plus retained monthly source snapshots. | Bounded TE calendar-page refresh into canonical `trading-storage` source data. | Active for recent/future schedule maintenance and reviewed historical macro calendar source artifacts. Source artifacts must not persist website URLs; M03 event-state materialization, not the feed, derives SQL event overview rows. |
| Official calendar discovery | Nasdaq earnings calendar shells, Nasdaq/S&P DJI index announcements, NYSE/Nasdaq exchange calendar artifacts. | Public official or reviewed local artifacts; live calls require manager controls. | Active artifact route for `calendar_observation`; Nasdaq earnings schedule refresh may share the calendar maintenance timer with TE; no direct M03 event-pool admission. |
