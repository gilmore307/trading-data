...
This document maps `trading-data` source-backed outputs to accepted `trading-model` M01-M06 consumers. It is an organization contract for external/source observations, not a complete training-data or derived-data plan.
...
`trading_data.option_chain_state_source` accepts manager-supplied `params.underlying`, `params.snapshot_time`, optional `params.window_start` / `params.window_end`, and option-chain scope controls, calls the ThetaData option selection snapshot interface once, and writes one contract-level source/cache row per visible contract per returned minute. Historical training acquisition is prepared by the manager before M02 only for targets whose metadata leaves listed options applicable; crypto and confirmed no-listed-options targets do not prepare this source or downstream M05 option-expression work. Provider policy caps remain `1d` while each task key carries the narrower 30-minute `window_start` / `window_end`. The resulting shared SQL rows serve M02 target-level reduction and M05 option-expression candidate preparation through `snapshot_time` range reuse.
