...
Current numbered data sources are cataloged in `src/data_models/catalog.py` and summarized here:
...
- `m01_market_regime_data_acquisition` — source package for the `trading_data.model_01_market_regime_data_acquisition` ETF bar SQL long table over the manager-supplied time range; ETF universe and grains come from `model_01_background_context_etf_universe.csv`.
- `m02_sector_context_data_acquisition` — retired ordinary-candidate route for SQL-only filtered US-listed equity ETF holdings evidence. It is not materialized by the current M02 feature stage and does not define the realtime total pool or historical replay candidate universe.
...
- `option_chain_state_source` — SQL-only shared ThetaData option-chain source/cache table; one row per visible contract per snapshot, consumed by M02 target-level option-chain reduction and M05 option-expression candidate preparation.
- `m03_event_state_data_acquisition` — M03 event-state source package; writes the SQL table `trading_data.model_03_event_state_data_acquisition`, one row per event, with details behind references.
- `m03_event_state_data_acquisition/equity_abnormal_activity` — nested event evidence detector package for equity/ETF abnormal activity rows.
