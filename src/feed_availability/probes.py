"""Concrete bounded feed availability probes."""
...
from .registry import FEEDS, FeedCandidate
...
def _parse_json_or_error(candidate: FeedCandidate, result: HttpResult) -> ProbeResult:
    try:
        payload = result.json()
...
def probe_us_treasury_fiscal_data(client: HttpClient, sec_user_agent: str) -> ProbeResult:
    del sec_user_agent
    candidate = FEEDS["us_treasury_fiscal_data"]
    result = client.get(
