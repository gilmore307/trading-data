...
- Source bars are canonical `1Min` rows from `source_01_market_regime`; this feature stage locally derives 30-minute and daily sector-context evidence from those source rows. `feature_bar_grain` remains the reviewed feature-grain contract, not a provider download contract.
