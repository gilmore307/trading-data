...
- `trading_data.data_bundles.*` accepts manager-issued task keys, chooses the required source interfaces, applies bundle config, and writes task-run outputs/receipts.
...
- `stock_etf_exposure` — ETF holdings snapshots + ETF/sector/theme config -> `stock_etf_exposure.csv`.
- `equity_abnormal_activity` — equity bars + optional benchmark/liquidity inputs -> `equity_abnormal_activity_event.csv`.
