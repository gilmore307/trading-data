...
GDELT is the primary broad news/event discovery source for U.S. and U.S.-market political, economic, war/geopolitical, and technology event candidates.
...
- **Default timestamp semantics:** `seen_at` is normalized to `America/New_York`; event projection should add `event_time` / `effective_time` when creating `trading_event` rows.
- **Natural grain:** One GDELT GKG article/source-evidence record.
- **Request parameters:** optional `query_terms`, `topic_categories`, `start_date`, `end_date`, `lookback_days`, `max_rows`, `search_fields`, `focus`, `source_domain_allowlist`, `source_domain_contains`, `impact_scope`. If `query_terms` is omitted, `topic_categories` defaults to `politics,economy,war,technology`.
