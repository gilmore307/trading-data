...
Historical and realtime/future event acquisition source priority is governed by `docs/23_event_source_registry.md`. Macro event observations use canonical Trading Economics storage rows as accepted source evidence; bounded TE recent/future refresh may append those rows, and M06 materialization derives `macro_data` event overview rows from reviewed TE storage artifacts. Market-session observations such as holidays, weekends, early closes, long non-trading intervals, option expirations, triple-witching windows, and ETF/index rebalance windows enter M06 materialization as `market_structure` context before attribution. Persistent regimes are promoted from repeated high-frequency news topics through agent review before becoming `persistent_event_regime` interval rows.
