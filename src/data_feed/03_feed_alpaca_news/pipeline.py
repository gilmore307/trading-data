...
def build_context(task_key,run_id):
    ...
    root=resolve_output_root(task_key, default_task_id="03_feed_alpaca_news_task"); run=root/'runs'/run_id
...
def fetch(context,*,client=None,client_is_fixture=False):
    ...
    if not client_is_fixture:
        require_provider_execution_allowed(context.task_key, provider='alpaca', endpoint_family='news', requested_symbols=len(symbols), requested_rows=int(params.get('limit',50))*max_pages, requested_requests=max_pages, requested_start=start, requested_end=end)
