...
- `price_action` is accepted as a M03 event-state category for false breakout / failed breakdown / liquidity sweep / bull-trap / bear-trap evidence; it remains event-risk evidence, not a new model layer or trading action.
...
- Event overlay sources are accepted through `model_03_event_state_data_acquisition`, including equity abnormal activity evidence.
...
- `macro_data` is active only through M03 event-state materialization from reviewed Trading Economics storage artifacts. Shared calendar maintenance may append recent/future TE calendar source rows and Nasdaq earnings schedule artifacts but must not persist TE website URLs or directly populate M03 event-state SQL rows.
