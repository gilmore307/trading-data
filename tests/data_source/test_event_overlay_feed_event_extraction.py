...
extract_events_from_artifact_paths = import_module(
    "data_source.source_09_event_risk_governor.feed_event_extraction"
).extract_events_from_artifact_paths
source_pipeline = import_module("data_source.source_09_event_risk_governor.pipeline")
...
class EventOverlayFeedExtractionTests(unittest.TestCase):
    ...
    def test_source_pipeline_accepts_event_artifact_paths(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            task_key = {
                "task_id": "source_09_event_risk_governor_artifact_task",
            ...
            self.assertEqual(result.row_counts["source_09_event_risk_governor"], 1)
    ...
    def test_source_pipeline_preserves_same_time_macro_calendar_events(self) -> None:
        with tempfile.TemporaryDirectory() as raw_tmp:
            ...
            task_key = {
                "task_id": "source_09_event_risk_governor_same_time_macro_task",
                "source": "source_09_event_risk_governor",
            ...
            result = source_pipeline.run(task_key, run_id="run", sql_writer=writer)
            self.assertEqual(result.status, "succeeded")
            self.assertEqual(result.row_counts["source_09_event_risk_governor"], 2)
