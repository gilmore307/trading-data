...
from trading_data.source_availability.__main__ import DEFAULT_SEC_USER_AGENT
...
def _required(mapping: Mapping[str, Any], key: str) -> Any:
    ...
    if value in (None, "", []):
        raise SecCompanyFinancialsError(f"{BUNDLE}.params.{key} is required")
    return value
...
def build_context(task_key: dict[str, Any], run_id: str) -> BundleContext:
    if task_key.get("bundle") != BUNDLE:
        raise SecCompanyFinancialsError(f"task_key.bundle must be {BUNDLE}")
...
def fetch(context: BundleContext, *, client: HttpClient | None = None, sec_user_agent: str = DEFAULT_SEC_USER_AGENT) -> tuple[StepResult, FetchedSecPayload]:
...
def clean(context: BundleContext, fetched: FetchedSecPayload) -> StepResult:
    params = dict(context.task_key.get("params") or {})
