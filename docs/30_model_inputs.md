...
`trading_data.m09_option_expression_data_acquisition` accepts manager-supplied `params.underlying`, `params.snapshot_time`, optional `params.window_start` / `params.window_end`, and optional `params.snapshot_type` (`entry`/`exit`, default `entry`), derives Layer 9 rows from `option_chain_state_source`, and writes one row per visible option contract per snapshot minute. `snapshot_time` is the point-in-time clock; quote/IV/Greeks provider row timestamps are intentionally omitted from the business table. `trading_data.m09_option_expression_feature_generation` turns accepted snapshot rows into deterministic source-only option-candidate payloads for moneyness, spread/liquidity, IV, Greeks, and quality diagnostics.
