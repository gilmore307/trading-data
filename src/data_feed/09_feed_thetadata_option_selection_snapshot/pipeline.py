...
DEFAULT_OPTION_BUCKET_POLICY_REF = "M05_OPTION_BUCKET_STRIKE_POLICY"
...
class RegistryNames:
    ...
    def field_name(self, ref: RegistryRef) -> str:
        try:
            ...
            raise ThetaDataOptionSelectionSnapshotError(f"stable field id not found: {ref.id}") from exc
...
def _required(mapping: Mapping[str, Any], key: str) -> Any:
    ...
    if value in (None, "", []):
        raise ThetaDataOptionSelectionSnapshotError(f"{FEED}.params.{key} is required")
    return value
...
def _parse_snapshot_time(value: Any) -> datetime:
    ...
    try:
        ...
        raise ThetaDataOptionSelectionSnapshotError(
            f"{FEED}.params.snapshot_time must be an ISO datetime"
        ) from exc
...
def build_context(task_key: dict[str, Any], run_id: str) -> FeedContext:
    if task_key.get("feed") != FEED:
        raise ThetaDataOptionSelectionSnapshotError(f"task_key.feed must be {FEED}")
...
def save(context: FeedContext, clean_result: StepResult, snapshot: dict[str, Any], *, sql_writer: SqlTableWriter | None = None) -> StepResult:
    ...
    fields = [
        ...
        names.field_name(SNAPSHOT_TIME),
