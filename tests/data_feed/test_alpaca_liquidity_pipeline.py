...
class AlpacaLiquidityPipelineTests(unittest.TestCase):
    ...
    def test_pipeline_accepts_replay_sample_windows(self):
        with tempfile.TemporaryDirectory() as tmp:
            task_key = {
        ...
        with tempfile.TemporaryDirectory() as tmp:
            task_key = {
                ...
                'params': {
                    ...
                    'end': '2024-02-01T00:00:00Z',
                    'timeframe': '1Min',
                    'replay_liquidity_acquisition_policy': 'full_hourly_regular_session_windows_per_component_month',
            ...
            receipt = json.loads((Path(task_key['output_root']) / 'completion_receipt.json').read_text())
            manifest = json.loads((Path(receipt['runs'][0]['output_dir']) / 'request_manifest.json').read_text())
            self.assertEqual(manifest['params']['replay_liquidity_acquisition_policy'], 'full_hourly_regular_session_windows_per_component_month')
