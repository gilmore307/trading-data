...
def _required(m,k):
    v=m.get(k)
    if v in (None,'',[]): raise AlpacaBarsError(f'01_source_alpaca_bars.params.{k} is required')
    return v
...
def build_context(task_key,run_id):
    if task_key.get('bundle')!='01_source_alpaca_bars': raise AlpacaBarsError('task_key.bundle must be 01_source_alpaca_bars')
    root=Path(str(task_key.get('output_root') or f"storage/{task_key.get('task_id','01_source_alpaca_bars_task')}")); run=root/'runs'/run_id
...
def write_receipt(context,*,status,fetch_result=None,clean_result=None,save_result=None,error=None):
    context.receipt_path.parent.mkdir(parents=True,exist_ok=True); existing={'task_id':context.task_key.get('task_id'),'bundle':'01_source_alpaca_bars','runs':[]}
    ...
    existing['runs']=[r for r in existing.get('runs',[]) if r.get('run_id')!=context.metadata['run_id']]+[entry]; existing.update({'task_id':context.task_key.get('task_id'),'bundle':'01_source_alpaca_bars'})
