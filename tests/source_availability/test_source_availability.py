...
class SourceAvailabilityTests(unittest.TestCase):
    ...
    def test_report_shape_and_write_path(self):
        ...
        self.assertEqual(payload["report_type"], "source_availability")
        self.assertEqual(payload["results"][0]["source"], "unit")
    ...
    def test_cli_list_mode_no_network(self):
        ...
        with redirect_stdout(stdout):
            exit_code = main(["--list", "--source", "bls"])
        self.assertEqual(exit_code, 0)
        payload = json.loads(stdout.getvalue())
        self.assertEqual(payload["sources"][0]["source"], "bls")
        self.assertIn("status_fields", payload)

    def test_cli_dry_run_no_write(self):
        stdout = io.StringIO()
        with redirect_stdout(stdout):
            exit_code = main(["--dry-run", "--source", "fred", "--no-write"])
        self.assertEqual(exit_code, 0)
        payload = json.loads(stdout.getvalue())
