...
- Output: in-memory feature rows for the single `feature_01_market_regime` table. SQL storage keeps one row per `snapshot_time` and stores generated feature values in `feature_payload_json` JSONB to avoid PostgreSQL row-size limits. Sector/industry rotation, daily-context pair features, sector-observation breadth/dispersion aggregates, and raw ratio moving-average level keys and standalone SHY return/trend keys are excluded; rotation evidence belongs to `feature_02_sector_context`, while normalized ratio distance/slope/spread/alignment features remain in Layer 1 when the pair is broad market/cross-asset evidence.
- No provider calls.
