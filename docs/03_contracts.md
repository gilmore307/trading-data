...
- `calendar_event_result`: post-release actual/consensus/surprise payloads with `released_at`, `available_time`, and `retrieved_at`; initial rows are materialized from accepted `source_10_event_risk_governor` result payloads without fabricating numeric surprises.
