...
| Data sources | Manager-facing orchestration for one accepted model-input or acquisition route. | `source_01_market_regime`, `source_02_target_candidate_holdings`, `source_03_target_state`, `source_05_option_expression`, `source_07_event_overlay`. |
...
| Instrument data / 标的数据 | Symbol, ETF holdings, liquidity, event, and target-candidate sources such as `source_02_target_candidate_holdings`, `source_03_target_state`, and `source_07_event_overlay`. |
