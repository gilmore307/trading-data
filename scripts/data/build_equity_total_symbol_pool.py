...
def build_pool(
    *,
    ...
    for row in rows_by_symbol.values():
        has_current_pool_source = row.in_dollar_volume_top300 or row.in_market_cap_top300
    ...
    receipt = {
        ...
        "boundary_note": "The CSV is the realtime equity total-symbol pool ledger built from TradingView traded-dollar-value and market-cap snapshots; active rows feed the calendar symbols file while inactive rows preserve currently observed but unusable symbols. Historical replay must use its frozen candidate-universe table instead of reading this mutable realtime pool directly.",
    }
    return rows, receipt
