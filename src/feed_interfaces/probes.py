...
from feed_availability.report import ProbeResult
from feed_availability.sanitize import sample_rows, sanitize_url, sanitize_value, shape_keys
...
def _candidate_like(interface: DataKindInterface):
    # ProbeResult.skipped expects FeedCandidate-like attributes. Keep this local
    # so feed_interfaces does not depend on feed_availability registry names.
    class Candidate:
        feed = interface.feed
        display_name = interface.feed
...
def _probe_okx(interface: DataKindInterface, client: HttpClient) -> ProbeResult:
    ...
    headers = {"User-Agent": "trading-source-feed-interfaces/0.1", "Accept": "application/json"}
...
def probe_many(data_kinds: list[str] | None, feed: str | None, client: HttpClient, *, sec_user_agent: str) -> list[ProbeResult]:
    selected = [item for item in INTERFACES.values() if (not feed or item.feed == feed) and (not data_kinds or item.data_kind in data_kinds)]
...
def interface_payload() -> list[dict[str, Any]]:
    return [asdict(item) for item in sorted(INTERFACES.values(), key=lambda value: (value.feed, value.data_kind))]
