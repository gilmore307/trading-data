...
- `feature_04_event_overlay/` — Layer 4 event-overlay source overview feature builder for deterministic event-category, scope, dedup, source-priority, and quality payloads; final `event_context_vector` remains in `trading-model`.
- `feature_08_option_expression/` — Layer 8 option-chain candidate feature builder for deterministic moneyness, spread/liquidity, IV, and Greeks payloads; final option-expression plans remain in `trading-model`.
