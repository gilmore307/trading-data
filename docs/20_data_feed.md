...
Without `--execute-live-fetch` it returns a plan-only receipt. With `--execute-live-fetch` it performs bounded calendar maintenance. The checked-in systemd unit runs once per day and passes `--schedule-te-release-fetches`: the daily TE pass discovers new or changed release-preview facts, writes monthly source buckets only when facts changed, and schedules one-shot release jobs at the known future release time. A release job starts immediately at the release clock, polls TE every 5 seconds for up to 60 seconds until an `actual` value is captured, and then writes canonical TE source rows. If no TE `actual` value appears within 60 seconds, it writes a `provisional_macro_release_web_search` artifact for realtime decision fallback; that artifact is not TE source and must be replaced by formal TE rows when they arrive. If the TE rows match existing monthly source evidence, the refresh reports `skipped_no_new_or_changed_rows` and leaves no run receipt or side-product files for that no-op. The NYSE artifact is used to overlay official NYSE/Nasdaq holiday and early-close rows into `trading_data.calendar_market_session`; the calendar-maintenance routes do not admit rows into the M06 event pool. The timer reads `/root/projects/trading-storage/main/shared/equity_total_symbol_pool.symbols.txt` by default, and `TRADING_DATA_CALENDAR_SYMBOLS_FILE` in `/etc/default/trading-data-calendar-maintenance` may override that path.
