...
def generate_rows(rows: Iterable[Mapping[str, Any]], *, run_id: str = "m10_event_risk_governor_feature_generation") -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        output.append(
            {
                "run_id": run_id,
                "source_run_ref": row.get("source_run_ref") or row.get("run_id") or "m10_event_risk_governor_data_acquisition",
