...
def generate_rows(rows: Iterable[Mapping[str, Any]], *, run_id: str = "feature_08_option_expression") -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        output.append(
            {
                "run_id": run_id,
                "source_run_ref": row.get("source_run_ref") or row.get("run_id") or "source_05_option_expression",
