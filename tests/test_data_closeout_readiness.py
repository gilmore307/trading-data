...
equity_activity_pipeline = importlib.import_module("data_source.source_08_event_risk_governor.equity_abnormal_activity.pipeline")
...
class DataCloseoutReadinessTests(unittest.TestCase):
    ...
    def test_equity_abnormal_activity_default_standard_is_conservative(self) -> None:
        config = json.loads(Path("src/data_source/source_08_event_risk_governor/equity_abnormal_activity/config.json").read_text())
    ...
    def test_source_config_loader_supports_nested_packaged_config(self) -> None:
        config = load_source_config("source_08_event_risk_governor/equity_abnormal_activity")
