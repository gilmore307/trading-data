...
NEWS_FIELDS=['id','headline','created_at_et','updated_at_et','symbols','summary','url']
...
def _required(m,k):
    v=m.get(k)
    if v in (None,'',[]): raise AlpacaNewsError(f'alpaca_news.params.{k} is required')
    return v
...
def clean(context,fetched):
    ...
    for i in fetched.news:
        rows.append({'id':i.get('id'),'headline':i.get('headline'),'created_at_et':_et_iso(i.get('created_at')),'updated_at_et':_et_iso(i.get('updated_at')),'symbols':i.get('symbols') or [],'summary':i.get('summary'),'url':i.get('url')})
...
def write_receipt(context,*,status,fetch_result=None,clean_result=None,save_result=None,error=None):
    ...
    existing['runs']=[r for r in existing.get('runs',[]) if r.get('run_id')!=context.metadata['run_id']]+[entry]; existing.update({'task_id':context.task_key.get('task_id'),'bundle':'alpaca_news'})
