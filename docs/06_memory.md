...
- Workflow decision: `trading-data` handles historical data only. `trading-manager` issues a self-contained data task key file, `trading-data` invokes the specified historical acquisition script with its parameters, writes cleaned rows to the specified storage SQL target once contracts are accepted, and writes a storage-resident task completion receipt. Realtime data remains `trading-execution` scope.
