...
- `feature_08_event_risk_governor/` — legacy-named Layer 8 event-risk-governor source overview feature builder for deterministic event-category, scope, dedup, source-priority, and quality payloads; final event-risk intervention remains in `trading-model` / execution risk-control.
- `feature_07_option_expression/` — legacy-named Layer 7 option-chain candidate feature builder for deterministic moneyness, spread/liquidity, IV, and Greeks payloads; final trading guidance / option-expression plans remain in `trading-model`.
