...
A small stdlib Python probe package now lives under `src/feed_availability/`.
...
PYTHONPATH=src python3 -m feed_availability --source bls --source us_treasury_fiscal_data
```

Live probe reports are written under `storage/feed_availability/`, which is ignored by Git. Reports contain probe status fields, HTTP status when available, response shape keys, and tiny sanitized sample rows only. They must not contain request headers, credential values, or full raw provider dumps.
...
The macro route writes sanitized request evidence, normalized rows, CSV/JSONL development outputs, and a completion receipt under ignored `storage/`. It does not persist full raw provider responses by default. The first live smoke runs confirmed these actual response shapes:
...
Do not create new manager routes to the removed `macro_data` route. Official macro API documentation and secret aliases may remain available for optional future research only.
...
| Source | Available categories | Access expectation | Notes |
|---|---|---|---|
| Alpaca | Equity bars, trades, quotes, snapshots, news. | `ALPACA_SECRET_ALIAS`; entitlement/feed availability must be checked at runtime. | News is its own feed. |
...
| OKX | Crypto bars; trades, quotes/tickers, and order book are available if later accepted. | Public market endpoints for market data; private endpoints need credentials. | Current accepted feed remains bars-focused. |
...
The source inventory now has an executable provider/data-kind layer under `src/feed_interfaces/`. This is separate from feed-level smoke probes: it maps each obtainable `data_kind` to a concrete feed, source, endpoint family, access rule, and bounded smoke request.

Common commands:

```bash
PYTHONPATH=src python3 -m feed_interfaces --list
PYTHONPATH=src python3 -m feed_interfaces --source alpaca
PYTHONPATH=src python3 -m feed_interfaces --source okx
PYTHONPATH=src python3 -m feed_interfaces --source 08_feed_sec_company_financials
PYTHONPATH=src python3 -m feed_interfaces --source thetadata
```

Live interface reports write under ignored `storage/feed_interfaces/` and contain sanitized endpoint/status/shape/sample evidence only.
...
`src/data_feed/01_feed_alpaca_bars/` now fetches Alpaca stock/ETF bars, normalizes timestamps to `America/New_York`, and saves cleaned `equity_bar` CSV outputs.

`src/data_feed/03_feed_alpaca_news/` now fetches Alpaca news, normalizes `created_at`/`updated_at` to `America/New_York`, and saves cleaned `equity_news` CSV outputs.

Both feeds use bounded pagination, sanitized request manifests, completion receipts, and no default full raw provider payload persistence.

## ThetaData option primary tracking implementation

`src/data_feed/10_feed_thetadata_option_primary_tracking/` now fetches specified-contract ThetaData option OHLC rows from the local Terminal v3 `/v3/option/history/ohlc` endpoint. It requires the caller to pass the contract (`underlying`, `expiration`, `right`, `strike`), `start_date`, `end_date`, and `timeframe`; it does not choose contracts.

The feed treats raw 1Sec OHLC rows as transient, skips zero-volume/count placeholders, aggregates active rows to the requested `America/New_York` timeframe, and persists only final `option_bar.csv` rows under `saved/`.

## ThetaData option event timeline implementation

`src/data_feed/11_feed_thetadata_option_event_timeline/` now fetches specified-contract ThetaData trade/quote rows from the local Terminal v3 `/v3/option/history/trade_quote` endpoint. It requires the caller to pass the contract, date range, event evidence-window `timeframe`, and a task/model `current_standard` object; the feed carries that event-time standard into each detail JSON rather than owning a global fixed threshold.

The feed groups transient trade/quote rows by ET evidence window, emits only windows where at least one supplied indicator standard is satisfied, saves final `option_activity_event.csv`, and writes one compact `<event_id>.json` `option_activity_event_detail` artifact per event. Raw provider rows and process/window state are not persisted by default.
