...
`fetch_tradingview_equity_screener.py` captures a bounded no-login TradingView US common-stock screener snapshot for volume, market cap, and sector evidence. `build_equity_total_symbol_pool.py` builds the shared pool from TradingView screener CSVs, Layer 2 ETF holdings CSVs, and an optionable-underlying symbol list. `refresh_equity_total_symbol_pool_from_tradingview.py` runs the TradingView snapshot and rebuilds the shared pool. Until a strict optionable source is supplied, its default mode keeps rows marked `uncertain_verify_before_use` and includes them in the calendar symbol text file; this is calendar/event-monitoring scope only, not proof that listed options are available for trading.
