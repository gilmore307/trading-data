...
- Scope: shared CSV combinations with `model_layer = layer_02_sector_context`; current reviewed rows use canonical `1m` `feature_bar_grain` values and `combination_type` values such as `sector_rotation` or `daily_context`.
