...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m feed_interfaces")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list:
        payload = {"interfaces": [item for item in interface_payload() if not args.feed or item["feed"] == args.feed]}
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 0
    results = probe_many(args.data_kind, args.feed, HttpClient(timeout_seconds=args.timeout_seconds), sec_user_agent=args.sec_user_agent)
    if args.no_write:
