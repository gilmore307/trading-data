...
from feed_interfaces.__main__ import main
from feed_interfaces.catalog import INTERFACES
from feed_interfaces.probes import probe_interface
...
class FeedInterfaceTests(unittest.TestCase):
    ...
    def test_trading_economics_is_macro_calendar_feed(self):
        ...
        self.assertEqual(interface.access, "web/storage-source")

    def test_cli_list_no_network(self):
        stdout = io.StringIO()
        with redirect_stdout(stdout):
            code = main(["--list", "--feed", "04_feed_okx_crypto_market_data"])
        self.assertEqual(code, 0)
        payload = json.loads(stdout.getvalue())
        self.assertTrue(payload["interfaces"])
        self.assertTrue(all(item["provider"] == "okx" for item in payload["interfaces"]))
        self.assertTrue(all(item["feed"] == "04_feed_okx_crypto_market_data" for item in payload["interfaces"]))
    ...
    def test_alpaca_probe_sanitizes_secret_summary(self):
        ...
        with patch("feed_interfaces.probes.load_secret_alias", return_value=Secret()):
