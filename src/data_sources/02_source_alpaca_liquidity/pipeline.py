...
def _required(mapping: dict[str, Any], key: str) -> Any:
    ...
    if value in (None, "", []):
        raise AlpacaLiquidityError(f"02_source_alpaca_liquidity.params.{key} is required")
    return value
...
def build_context(task_key: dict[str, Any], run_id: str) -> BundleContext:
    if task_key.get("bundle") != "02_source_alpaca_liquidity":
        raise AlpacaLiquidityError("task_key.bundle must be 02_source_alpaca_liquidity")
    output_root = Path(str(task_key.get("output_root") or f"storage/{task_key.get('task_id', '02_source_alpaca_liquidity_task')}"))
...
def write_receipt(context: BundleContext, *, status: str, fetch_result: StepResult | None = None, clean_result: StepResult | None = None, save_result: StepResult | None = None, error: BaseException | None = None) -> StepResult:
    ...
    existing = {"task_id": context.task_key.get("task_id"), "bundle": "02_source_alpaca_liquidity", "runs": []}
    ...
    existing.update({"task_id": context.task_key.get("task_id"), "bundle": "02_source_alpaca_liquidity"})
