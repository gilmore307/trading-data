...
| `OptionExpressionModel` | `trading_data.option_chain_state_source`, `trading_data.m09_option_expression_data_acquisition`, `trading_data.m09_option_expression_feature_generation` | shared contract-level option-chain source/cache, Layer 9 entry/exit snapshots, and deterministic option-candidate feature payloads | `trading-data` prepares moneyness, spread/liquidity, IV, Greeks, and quality payloads from the shared option-chain rows; `trading-model` owns contract ranking and expression choice. |
...
`trading_data.option_chain_state_source` accepts manager-supplied `params.underlying`, `params.snapshot_time`, and option-chain scope controls, calls the ThetaData option selection snapshot interface once, and writes one contract-level source/cache row per visible contract per snapshot. It is the shared option-chain source for Layer 3 target-level reduction and Layer 9 option-expression candidate preparation.

`trading_data.m09_option_expression_data_acquisition` accepts manager-supplied `params.underlying`, `params.snapshot_time`, and optional `params.snapshot_type` (`entry`/`exit`, default `entry`), derives Layer 9 rows from `option_chain_state_source`, and writes one row per visible option contract per entry/exit snapshot. `snapshot_time` is the point-in-time clock; quote/IV/Greeks provider row timestamps are intentionally omitted from the business table. `trading_data.m09_option_expression_feature_generation` turns accepted snapshot rows into deterministic source-only option-candidate payloads for moneyness, spread/liquidity, IV, Greeks, and quality diagnostics.
