"""Manager-facing 03 StrategySelectionModel bar/liquidity input bundle."""
...
def build_context(task_key: dict[str, Any], run_id: str) -> BundleContext:
    if task_key.get("bundle") != BUNDLE:
        raise StrategySelectionInputsError(f"task_key.bundle must be {BUNDLE}")
...
def _row(context: BundleContext, symbol: str, timeframe: str, timestamp: str, bar: Mapping[str, Any], liquidity: Mapping[str, Any]) -> dict[str, Any]:
