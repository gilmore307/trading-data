# m03_event_state_data_acquisition/equity_abnormal_activity

Derived event detector inside the `m03_event_state_data_acquisition` M03 EventState surface. This is not a standalone manager-facing numbered data source.

It converts SQL-retained equity/ETF bars, optional reference bars, and optional liquidity rows into compact `equity_abnormal_activity_event` evidence rows inside the M03 event-state evidence surface. M03 event-state may consume only reviewed/promoted evidence packets, not raw abnormal-activity rows.
