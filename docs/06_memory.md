...
- Macro acquisition now uses one accepted bundle key, `macro_data`, for FRED-unique series, Census, BEA, BLS, U.S. Treasury Fiscal Data, and official macro source pages. Task params select the concrete provider/source, dataset/release/series, cadence, period, source URL, and output target.
