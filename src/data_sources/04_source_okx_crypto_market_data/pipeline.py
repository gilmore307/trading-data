...
def _required(mapping: dict[str, Any], key: str) -> Any:
    ...
    if value in (None, "", []):
        raise OkxCryptoMarketDataError(f"04_source_okx_crypto_market_data.params.{key} is required")
    return value
...
def build_context(task_key: dict[str, Any], run_id: str) -> BundleContext:
    if task_key.get("bundle") != "04_source_okx_crypto_market_data":
        raise OkxCryptoMarketDataError("task_key.bundle must be 04_source_okx_crypto_market_data")
    output_root = Path(str(task_key.get("output_root") or f"storage/{task_key.get('task_id', '04_source_okx_crypto_market_data_task')}"))
...
def write_receipt(context: BundleContext, *, fetch_result: StepResult, clean_result: StepResult, save_result: StepResult, error: str | None = None) -> StepResult:
    existing = {"task_id": context.task_key.get("task_id"), "bundle": "04_source_okx_crypto_market_data", "runs": []}
    ...
    existing.update({"task_id": context.task_key.get("task_id"), "bundle": "04_source_okx_crypto_market_data"})
