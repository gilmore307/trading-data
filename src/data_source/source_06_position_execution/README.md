...
This source supports Layer 8 OptionExpressionModel replay/evaluation by collecting the market path of contracts selected by an upstream offline option-expression plan. Despite the historical `source_06` identifier, it is not broker execution, not Layer 6 `PositionProjectionModel`, not a model-output layer, and does not emit execution instructions. The source accepts multiple selected contracts and writes option contract time-series rows from each contract's entry time through one hour after its exit time.
