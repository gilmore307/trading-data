...
- `macro_data` has been removed as an executable macro acquisition bundle. Macro model inputs now use `trading_economics_calendar_web` visible-page rows; official macro API secret aliases may remain stored for optional future research but are not active manager task routes.
