# M01 Sector Context Data

This file records the `trading-data` responsibility for M02. It is intentionally narrow: model interpretation belongs to `trading-model`, and global naming authority belongs to `trading-manager`.
...
M02 currently consumes a deterministic feature surface built from eligible broad sector-anchor ETF market behavior. There is no accepted separate sector-context source artifact.
...
The reviewed M02 ETF universe is restricted to the 11 broad Select Sector SPDR anchors plus the `BKCH` crypto context-anchor exception. Focused industry-chain and theme ETFs are not M02 sector anchors. ETF holdings and `stock_etf_exposure` are not core behavior inputs for the SectorContextModel and do not define the ordinary equity candidate universe. Ordinary candidates come from the reviewed total-symbol pool and target metadata; M02 supplies broad sector context attached to those candidates.
...
Raw/source columns may use clear provider or observation names without a layer prefix when they are generic facts. Model-owned feature or model-facing keys use canonical compact prefixes when they represent M02 concepts, for example `2_trend_stability_score` and `2_sector_handoff_state`. Do not introduce `layer02_*` aliases for the same concept.

## Stage flow

```mermaid
flowchart LR
    request["trading-manager task/request<br/>M02 sector context feature need"]
    ...
    downstream["M02 target-state input preparation<br/>attaches broad sector context to candidates"]
...
M02 data changes are acceptable when they:

- keep `m02_sector_context_feature_generation` as the deterministic M02 feature surface;
...
- keep focused industry/theme ETFs out of M02 unless a separate reviewed proxy/theme layer is accepted, while preserving the `BKCH` crypto context-anchor exception;
...
- support the three target-context routing cases defined by the model contract: M01 ETF target, M02 context ETF target, and ordinary target with dynamic context-profile weighting;
