"""OKX crypto market-data acquisition and cleaning feed."""
...
def _required(mapping: dict[str, Any], key: str) -> Any:
    ...
    if value in (None, "", []):
        raise OkxCryptoMarketDataError(f"04_feed_okx_crypto_market_data.params.{key} is required")
    return value
...
def fetch(context: FeedContext, *, client: HttpClient | None = None) -> tuple[StepResult, FetchedPayload]:
    ...
    headers = {"User-Agent": "trading-source-04-feed-okx-crypto-market-data/0.1", "Accept": "application/json"}
...
def write_receipt(context: FeedContext, *, fetch_result: StepResult, clean_result: StepResult, save_result: StepResult, error: str | None = None) -> StepResult:
    existing = {"task_id": context.task_key.get("task_id"), "feed": "04_feed_okx_crypto_market_data", "runs": []}
