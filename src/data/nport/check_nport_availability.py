...
from src.data.common.business_time import business_month_key, business_now_iso
...
EARLIEST_SUPPORTED_MONTH = "2019-01"


def now_iso() -> str:
    return business_now_iso()
...
def load_state(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "target_month": business_month_key(),
...
def main() -> None:
    ...
    target_month = args.target_month or state.get("target_month") or business_month_key()
    ...
    if target_month < EARLIEST_SUPPORTED_MONTH:
        outcome["availability_signal"] = f"target month is earlier than supported N-PORT coverage floor {EARLIEST_SUPPORTED_MONTH}"
        outcome["coverage_floor"] = EARLIEST_SUPPORTED_MONTH
        outcome["status"] = "not_applicable_pre_nport"
