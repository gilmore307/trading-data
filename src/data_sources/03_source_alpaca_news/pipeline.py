...
def _required(m,k):
    v=m.get(k)
    if v in (None,'',[]): raise AlpacaNewsError(f'03_source_alpaca_news.params.{k} is required')
    return v
...
def build_context(task_key,run_id):
    if task_key.get('bundle')!='03_source_alpaca_news': raise AlpacaNewsError('task_key.bundle must be 03_source_alpaca_news')
    root=Path(str(task_key.get('output_root') or f"storage/{task_key.get('task_id','03_source_alpaca_news_task')}")); run=root/'runs'/run_id
...
def write_receipt(context,*,status,fetch_result=None,clean_result=None,save_result=None,error=None):
    context.receipt_path.parent.mkdir(parents=True,exist_ok=True); existing={'task_id':context.task_key.get('task_id'),'bundle':'03_source_alpaca_news','runs':[]}
    ...
    existing['runs']=[r for r in existing.get('runs',[]) if r.get('run_id')!=context.metadata['run_id']]+[entry]; existing.update({'task_id':context.task_key.get('task_id'),'bundle':'03_source_alpaca_news'})
