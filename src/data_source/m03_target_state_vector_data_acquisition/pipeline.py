"""Deterministic M03 target-state-vector data-acquisition normalizer.
...
def clean(context: SourceContext, payload: SourcePayload) -> tuple[StepResult, CleanedPayload]:
    ...
    if not rows:
        raise TargetStateSourceError("m03_target_state_vector_data_acquisition produced zero candidate-mapped rows")
    result = StepResult(
        "succeeded",
        [],
