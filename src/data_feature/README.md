...
- `feature_03_strategy_variant_simulation/` — planned request-driven Layer 3 deterministic strategy variant simulation feature surface for `trading_data.feature_03_strategy_variant_simulation`; `trading-manager` issues the run request, `trading-data` simulates reviewed family/variant specs, and `trading-model` consumes the output for oracle construction, variant lifecycle review, and promotion evidence.
