...
`trading_data.m05_option_expression_feature_generation` reads `option_chain_state_source` rows directly and turns accepted point-in-time snapshots into deterministic source-only option-candidate payloads for moneyness, spread/liquidity, IV, Greeks, and quality diagnostics.

`trading_data.m05_option_expression_data_acquisition_contract_path` accepts `params.selected_contracts` from the option-expression handoff and writes selected option contract market data from entry time through exit time plus one hour. It emits market data only and must not produce execution instructions, order fields, PnL labels, or model outputs.
...
- `trading_data.option_chain_state_source` plus `trading_data.m05_option_expression_feature_generation` are the option-expression input surfaces; they are not Layer 5 `AlphaConfidenceModel` acquisition.
