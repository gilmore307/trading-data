...
- Registered config `TRADING_SOURCE_DEVELOPMENT_STORAGE_ROOT` points to `storage`.
...
A development database smoke test attempted to materialize all `feature_01_market_regime` generated feature columns as physical `DOUBLE PRECISION` columns. PostgreSQL rejected dense rows with `row is too big: size 8768, maximum size 8160`. The V1 feature generator originally emitted 1,477 generated feature values per snapshot, and after the sector-rotation split it emits 959 Layer 1 generated feature values per snapshot. A physically wide table is still not the durable contract for dense model-local feature surfaces.
...
Candidate-comparison rows store relative-strength return, trend, volatility-ratio, and correlation evidence in `feature_payload_json`. The sector summary row stores sector-observation breadth and dispersion evidence in `feature_payload_json`.
...
- Current shared-contract width for `feature_01_market_regime` is 960 dictionary fields including `snapshot_time` (959 payload keys).
- Current shared-contract row count for `feature_02_security_selection` is 31 rows per snapshot: 1 `sector_rotation_summary` row, 17 `sector_rotation` candidate-comparison rows, and 13 `daily_context` candidate-comparison rows.
