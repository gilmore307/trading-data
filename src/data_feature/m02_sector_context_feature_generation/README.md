...
M02 `SectorContextModel` sector/industry behavior-evidence feature generator.
...
- Scope: shared CSV combinations with `model_layer = model_01_sector_context`; current reviewed rows use canonical `1m` `feature_bar_grain` values and `combination_type` values such as `sector_rotation` or `context_rotation`.
...
This module owns deterministic evidence for M02 conditional ETF-context behavior under market context. M01 should not carry sector/industry rotation evidence; sector leadership, sector-vs-sector comparison, and sector-observation participation belong here.
